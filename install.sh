#!/bin/bash
# Bash Script tp install Fsociety tools
...
echo "if your os is on arch or fedora, install git and python2 manually via pacman (if arch) or yum (if fedora/centos)"
    sudo apt-get install -y git python2
fi
...
echo "[✔] Installing ...";
echo "";
git clone --depth=1 https://github.com/MiRw3b/fsociety-fixed/ "$INSTALL_DIR";
echo "#!$BASH_PATH
...
echo "Installed!"
echo "You may now run fsociety.py with python"
exit